...
export const useItemHighlight = () => {
  ...
  const categoryCache = useRef<Map<string, { id: string; name: string }>>(
    new Map(),
  )
  ...
  const switchToCategory = useCallback((categoryName: string) => {
    try {
      ...
      for (const tab of allTabs) {
        if (
          tab.textContent?.trim().toLowerCase() === categoryName.toLowerCase()
        ) {
  ...
  const applyHighlightEffect = useCallback(
    (element: HTMLElement, options: HighlightOptions = {}) => {
      const { highlightDuration = 2500, scrollBehavior = 'smooth' } = options
  ...
  const highlightItem = useCallback(
    async (
      ...
      const { maxRetries = 8, retryCount = 0 } = options
      ...
      try {
        ...
        if (resolvedCategoryName && resolvedCategoryName !== 'All') {
          ...
          if (!tabSwitched) {
            console.warn(
              `Failed to switch to category: ${resolvedCategoryName}`,
            )
        ...
        const attemptHighlight = (currentRetry = 0) => {
          ...
          if (itemElement) {
            console.log(
              `Successfully found item element on attempt ${currentRetry + 1}`,
            )
            applyHighlightEffect(itemElement, options)
          } else if (currentRetry < maxRetries) {
            console.log(
              `Item not found, retrying... (${currentRetry + 1}/${maxRetries})`,
            )

            // Exponential backoff with jitter
            const delay = Math.min(
              300 + currentRetry * 200 + Math.random() * 100,
              2000,
            )
            ...
            console.warn(
              `Could not find element with id: food-item-${itemId} after ${maxRetries} attempts`,
            )
            setIsHighlighting(false)
            ...
            if (resolvedCategoryName) {
              const categorySection = document.querySelector(
                `[data-category="${resolvedCategoryName}"]`,
              )
              if (categorySection) {
  ...
  const highlightMultipleItems = useCallback(
    async (
      items: Array<{ id: string; categoryId?: string; categoryName?: string }>,
    ) => {
