...
export default function UseOptimizeImage({
  ...
  width = 100,
  height = 100,
  children, // Destructure children to prevent passing it to void element
  ...otherProps
}: UseOptimizeImageProps & { children?: React.ReactNode }) {
  ...
  if (!src) {
    return (
      <div
        className={`relative w-full h-full overflow-hidden ${rounded} bg-gray-200 animate-pulse`}
      />
    )
  }
