...
import { fuzzySearch, SearchResult, InventoryItem } from '@/helpers/fuzzySearch'
...
export const useSearchQuery = () => {
  ...
  const dataLoading = inventoryLoading || categoriesLoading
  ...
  // Build category map when categories change
  useEffect(() => {
  ...
  const isCacheValid = useCallback(
    (cacheEntry: SearchCache): boolean => {
  ...
  const performSearch = useCallback(
      ...
      searchTimeoutRef.current = setTimeout(() => {
        ...
        try {
          ...
          if (!allItems || allItems.length === 0) {
            setSearchResults([])
            if (dataLoading) {
              setInternalError('Loading menu items...')
            } else {
              setInternalError(
          ...
          if (validItems.length === 0) {
            setSearchResults([])
            setInternalError('Invalid menu data. Please refresh the page.')
            setIsLoading(false)
            return
          }
          ...
          setInternalError(null) // Clear any previous errors
          ...
          if (err instanceof TypeError) {
            setInternalError('Invalid search data. Please refresh the page.')
          } else if (err instanceof RangeError) {
            setInternalError(
              'Search query too complex. Please try a simpler search.',
            )
          } else {
            setInternalError(
    ...
    [allItems, cleanupCache, dataLoading, isCacheValid],
  )
  ...
  return {
    ...
    error: internalError || inventoryError || categoriesError,
    performSearch,
    clearSearch,
    getCategoryForItem,
