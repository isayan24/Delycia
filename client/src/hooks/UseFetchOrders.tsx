...
export default function UseFetchOrders(
  ...
  const [isAutoRefreshActive, setIsAutoRefreshActive] =
    useState<boolean>(enableAutoRefresh)
   ...
   */
  const filterOrdersByRid = useCallback(
    (ordersData: Order[], restaurantId?: string | null) => {
      if (!restaurantId) {
    ...
    async (showLoadingState: boolean = true) => {
      ...
      try {
        ...
        if (response.data?.statusCode === 200 && response.data?.orders) {
          ...
          // Filter by RID if provided
          const filteredOrders = filterOrdersByRid(ordersData, rid)
          ...
          const totalOrdersText = rid
            ? `${filteredOrders.length} orders for restaurant (${ordersData.length} total)`
