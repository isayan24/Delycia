  ...
  const cartItemUI = [
    {
      ...
      cartItemTextUI:
        'flex flex-col text-white max-[500px]:py-0 max-[700px]:gap-2 ',
      cartItemName:
        'text-xl font-[500] text-white max-[500px]:text-[15px] max-[700px]:!leading-[15px]',
      cartItemDesc:
      ...
      <form className="">
        {showCartItems.map((item: any) => (
          <div key={item.id}>
              ...
              <main className={`${cartItemUI[0].cartItemUI} `}>
                <div className="flex gap-4 max-[700px]:ml-6">
                  ...
                  >
                    ...
                    <div>
                      <h1
                        className={`${cartItemUI[0].cartItemName} text-black!`}
                      >
                        {item?.name}
                      </h1>
                    ...
                    <section className="flex items-center gap-2 mt-4 max-[500px]:mt-1">
                      ...
                      >
                        {item.quantity > 1 ? (
                          ...
                          hover:bg-linear-to-r ${item.status === 'out_of_stock' ? 'cursor-not-allowed pointer-events-none' : 'cursor-pointer'}`}
                          />
                        ) : (
                          <Trash
                            onClick={() => handleRemove(item.id)}
                            className={`${cartItemUI[0].quantityBtnIncreaseUI} my-auto hover:bg-linear-to-r max-[500px]:w-[2.8rem]!`}
                          />
                        )}

                        <span className=" max-[500px]:text-[12px]">
                          {item.quantity}
                        </span>
                        <Plus
                          onClick={() =>
                          ...
                          className={`${cartItemUI[0].quantityBtnIncreaseUI} hover:bg-linear-to-l ${
                            item.status === 'out_of_stock'
                              ? 'cursor-not-allowed'
                              : 'cursor-pointer'
                          }`}
            ...
            <hr className="w-140 mt-6 bg-gray-200 max-[700px]:w-[80%] mx-auto" />
          </div>
        ))}
      </form>
    </Form>
