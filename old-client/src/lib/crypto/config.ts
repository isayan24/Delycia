...
class CryptoConfigManager {
  ...
  private loadConfiguration(): CryptoConfig {
    try {
      ...
      if (!secretKey) {
        console.warn('CRYPTO_SECRET_KEY not found in environment variables, using default');
      }

      const config: CryptoConfig = {
        SECRET_KEY: secretKey || 'iuJhefiiKxerEPPMwjjSUHRTZfumDMmqQAFGVePaNvcRkkgxvz',
        SIGNATURE_HEADER: 'x-signature',
        ORDERS_ENDPOINT: '/orders'
      };

      console.info('Crypto configuration loaded successfully', {
      ...
      console.error('Failed to load crypto configuration', { error: errorMessage });
  ...
  public validateConfiguration(): boolean {
    try {
      ...
      if (!isValid) {
        ...
        console.debug('Crypto configuration validation passed');
      }

      return isValid;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('Error during configuration validation', { error: errorMessage });
      return false;
    }
  }
  ...
  public reloadConfiguration(): void {
    console.info('Reloading crypto configuration');
    this.config = this.loadConfiguration();
  }
}
