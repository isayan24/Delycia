...
export function useAuth(): UseAuthReturn {
  ...
  const login = useCallback(async (credentials: LoginCredentials): Promise<boolean> => {
    try {
      ...
      const response = await axiosInstance.post("/users/auth/handleAuth", credentials);
      ...
      if (response.data?.data) {
        ...
        try {
          const codeResult = await submitCodeAutomatically(sessionService.getUserData());

          if (codeResult.success) {
            console.log("Login: Code automatically submitted successfully");
          }
        } catch (codeError) {
          console.error("Login: Unhandled error during automatic code submission:", codeError);
        }
