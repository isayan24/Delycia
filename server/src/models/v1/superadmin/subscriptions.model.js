...
const getAllPlans = async (req) => {
  try {
    ...
    const query = `
      ...
      LEFT JOIN subscriptions sa ON sp.id = sa.plan_id AND sa.status = 'active'
      ${whereClause}
      GROUP BY sp.id
    ...
    return apiResponse.success(200, "Subscription plans retrieved successfully", {
      data: plans,
    });
  } catch (error) {
    console.error("Get all plans error:", error);
    return apiResponse.error(500, "An error occurred while retrieving subscription plans");
  }
};
...
const getPlanById = async (req) => {
  try {
    ...
    if (!id) {
      return apiResponse.error(400, "Plan ID is required");
    }
    ...
    return apiResponse.success(200, "Subscription plan retrieved successfully", {
      data: result[0],
    });
  } catch (error) {
    console.error("Get plan error:", error);
    return apiResponse.error(500, "An error occurred while retrieving subscription plan");
  }
};
...
const createPlan = async (req) => {
  try {
    ...
    if (!plan_code || !plan_name || price === undefined || !billing_period || !billing_days) {
      return apiResponse.error(400, "Missing required fields: plan_code, plan_name, price, billing_period, billing_days");
    }

    // Validate billing_period
    const validBillingPeriods = ["month", "year", "trial"];
    if (!validBillingPeriods.includes(billing_period)) {
      return apiResponse.error(400, "Invalid billing_period. Must be one of: month, year, trial");
    }

    // Validate price is positive
    if (price < 0) {
      return apiResponse.error(400, "Price must be a positive number");
    }

    // Require at least one feature
    if (!features || !Array.isArray(features) || features.length === 0) {
      return apiResponse.error(400, "At least one feature must be defined");
    }
    ...
    if (existingCode.length > 0) {
      return apiResponse.error(409, "Plan code already exists");
    }
    ...
    return apiResponse.success(201, "Subscription plan created successfully", {
      data: newPlan[0],
    });
  } catch (error) {
    console.error("Create plan error:", error);
    return apiResponse.error(500, "An error occurred while creating subscription plan");
  }
};
  ...
  try {
    ...
    if (updateData.plan_name && updateData.plan_name !== existing[0].plan_name) {
      ...
      if (existingName.length > 0) {
        return apiResponse.error(409, "Plan name already exists");
      }
    }
    ...
    return apiResponse.success(200, "Subscription plan updated successfully. Changes will apply to new subscriptions only.", {
      data: updatedPlan[0],
    });
  } catch (error) {
    console.error("Update plan error:", error);
    return apiResponse.error(500, "An error occurred while updating subscription plan");
  }
};
 ...
 */
const deactivatePlan = async (req) => {
  try {
    const { id } = req.params;
    ...
    if (existing[0].is_active === 0) {
      return apiResponse.error(400, "Subscription plan is already inactive");
    }
    ...
    return apiResponse.success(200, "Subscription plan deactivated successfully. Existing subscriptions will continue until their end date.", {
      data: deactivatedPlan[0],
    });
  } catch (error) {
    console.error("Deactivate plan error:", error);
    return apiResponse.error(500, "An error occurred while deactivating subscription plan");
  }
};
...
const assignPlan = async (req) => {
  ...
  try {
    ...
    if (!restaurant_id || !subscription_plan_id || !start_date) {
      return apiResponse.error(400, "Missing required fields: restaurant_id, subscription_plan_id, start_date");
    }
    ...
    if (end_date) {
      ...
      if (endDateObj <= startDateObj) {
        return apiResponse.error(422, "End date must be after start date");
      }
    }
    ...
    return apiResponse.success(201, "Subscription assigned successfully", {
      data: newAssignment[0],
    });
  } catch (error) {
    console.error("Assign plan error:", error);
    return apiResponse.error(500, "An error occurred while assigning subscription plan");
  }
};
 ...
 */
const getRestaurantSubscription = async (req) => {
  try {
    ...
    if (!subscription.length) {
      return apiResponse.error(404, "No active subscription found for this restaurant");
    }
    ...
    return apiResponse.success(200, "Restaurant subscription retrieved successfully", {
      data: subscriptionData,
    });
  } catch (error) {
    console.error("Get restaurant subscription error:", error);
    return apiResponse.error(500, "An error occurred while retrieving restaurant subscription");
  }
};
 ...
 */
const getSubscriptionHistory = async (req) => {
  try {
    const { restaurantId } = req.params;
    ...
    return apiResponse.success(200, "Subscription history retrieved successfully", {
      data: {
    ...
    return apiResponse.error(500, "An error occurred while retrieving subscription history");
  }
};
...
const changePlan = async (req) => {
  try {
    ...
    if (!id) {
      return apiResponse.error(400, "Assignment ID is required");
    }
    ...
    if (currentAssignment[0].plan_id === subscription_plan_id) {
      return apiResponse.error(400, "Restaurant is already on this subscription plan");
    }
    ...
    return apiResponse.error(500, "An error occurred while changing subscription plan");
  }
};
