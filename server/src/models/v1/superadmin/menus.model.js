...
const getAllMenus = async (req) => {
  try {
    ...
    // Get menu items with restaurant and category info
    const query = `
      SELECT 
        i.id,
    ...
    return apiResponse.success(200, "Menu items retrieved successfully", {
      data: formattedItems,
      pagination: {
        page: parseInt(page),
    ...
    return apiResponse.error(500, "An error occurred while retrieving menu items");
  }
};
...
const getMenuItemById = async (req) => {
  try {
    ...
    if (!id) {
      return apiResponse.error(400, "Menu item ID is required");
    }
    ...
    if (!result.length) {
      return apiResponse.error(404, "Menu item not found");
    }
    ...
    return apiResponse.success(200, "Menu item details retrieved successfully", {
      data: menuItem,
    });
  } catch (error) {
    console.error("Get menu item error:", error);
    return apiResponse.error(500, "An error occurred while retrieving menu item details");
  }
};
  ...
  try {
    ...
    if (updateData.price !== undefined) {
      ...
      if (isNaN(price) || price <= 0) {
        return apiResponse.error(400, "Price must be a positive number greater than zero");
      }
    }
    ...
    if (updateData.cost !== undefined) {
      ...
      if (isNaN(cost) || cost < 0) {
        return apiResponse.error(400, "Cost must be a non-negative number");
      }
    }
    ...
    return apiResponse.success(200, "Menu item updated successfully", {
      data: menuItem,
    });
  } catch (error) {
    console.error("Update menu item error:", error);
    return apiResponse.error(500, "An error occurred while updating menu item");
  }
};
...
const deleteMenuItem = async (req) => {
  try {
    ...
    if (existing[0].status === "unavailable") {
      return apiResponse.error(400, "Menu item is already marked as unavailable (deleted)");
    }

    // Soft delete: Mark as unavailable to preserve historical order data
    await pool.query(
    ...
    return apiResponse.error(500, "An error occurred while deleting menu item");
  }
};
...
const createCategory = async (req) => {
  try {
    ...
    if (!rid || !name) {
      return apiResponse.error(400, "Missing required fields: rid (restaurant_id), name");
    }
    ...
    if (restaurant[0].is_active === 0) {
      return apiResponse.error(400, "Cannot create category for inactive restaurant");
    }
    ...
    if (existingCategory.length > 0) {
      return apiResponse.error(409, "Category with this name already exists for this restaurant");
    }
    ...
    return apiResponse.error(500, "An error occurred while creating category");
  }
};
...
const updateCategory = async (req) => {
  try {
    ...
    if (!id) {
      return apiResponse.error(400, "Category ID is required");
    }
    ...
    return apiResponse.error(500, "An error occurred while updating category");
  }
};
...
const deleteCategory = async (req) => {
  try {
    ...
    return apiResponse.error(500, "An error occurred while deleting category");
  }
};
 ...
 */
const bulkUpdateMenuItems = async (req) => {
  try {
    const { items } = req.body;

    if (!items || !Array.isArray(items) || items.length === 0) {
      return apiResponse.error(400, "Items array is required and must contain at least one item");
    }
    ...
    if (validationErrors.length > 0) {
      return apiResponse.error(400, "Validation errors", {
    ...
    if (existingItems.length !== itemIds.length) {
      ...
      return apiResponse.error(404, "Some menu items not found", {
    ...
    return apiResponse.error(500, "An error occurred while performing bulk update");
  }
};
