...
const getAllStaff = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Staff retrieved successfully", {
        data: formattedStaff,
        pagination: {
          page: parseInt(page),
    ...
    return res
      .status(500)
      .json(
        apiResponse.error(500, "An error occurred while retrieving staff")
      );
  }
};
...
const getStaff = async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json(apiResponse.error(400, "Staff ID is required"));
    }
    ...
    if (!staffResult.length) {
      return res.status(404).json(apiResponse.error(404, "Staff member not found"));
    }
    ...
    return res.status(200).json(
      apiResponse.success(200, "Staff details retrieved successfully", {
        data: staff,
      })
    );
  } catch (error) {
    ...
    return res
      ...
      .json(
        apiResponse.error(
          500,
          "An error occurred while retrieving staff details"
        )
      );
  }
};
  ...
  try {
    ...
    if (!name || !restaurant_id) {
      return res
        .status(400)
        .json(
          apiResponse.error(400, "Missing required fields: name, restaurant_id")
        );
    }
    ...
    if (role < 2 || role > 7) {
      return res
        .status(400)
        .json(
          apiResponse.error(400, "Invalid role. Staff role must be between 2 and 7")
        );
    }
    ...
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return res
        .status(400)
        .json(apiResponse.error(400, "Invalid email format"));
    }

    // Verify restaurant exists and is active - Requirement 6.8
    ...
    if (restaurant[0].is_active === 0) {
      return res
        ...
        .json(
          apiResponse.error(
            400,
            "Cannot assign staff to inactive restaurant"
          )
        );
    }

    // Check email uniqueness within restaurant - Requirement 6.7
    if (email) {
    ...
    if (username) {
      ...
      if (existingUsername.length > 0) {
        return res
          .status(409)
          .json(apiResponse.error(409, "Username already exists"));
      }
    }
    ...
    return res.status(201).json(
      apiResponse.success(201, "Staff member created successfully. Credentials have been sent to the staff's email.", {
        data: staffData,
      })
    );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(apiResponse.error(500, "An error occurred while creating staff member"));
  }
};
...
const updateStaff = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Staff member updated successfully", {
        data: staffData,
      })
    );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(apiResponse.error(500, "An error occurred while updating staff member"));
  }
};
...
const deactivateStaff = async (req, res) => {
  try {
    ...
    if (existing[0].role === 0) {
      return res
        .status(400)
        .json(apiResponse.error(400, "Staff member is already inactive"));
    }
    ...
    return res
      .status(500)
      .json(
        apiResponse.error(500, "An error occurred while deactivating staff member")
      );
  }
};
...
const getStaffActivity = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Staff activity retrieved successfully", {
        data: {
    ...
    return res
      ...
      .json(
        apiResponse.error(
          500,
          "An error occurred while retrieving staff activity"
        )
      );
  }
};
