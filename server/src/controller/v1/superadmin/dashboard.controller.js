 ...
 * Requirement 12.1: Display total restaurant count, active subscriptions, and revenue metrics
 */
const getStats = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Dashboard statistics retrieved successfully", {
        data: {
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while retrieving dashboard statistics")
    );
  }
};
 ...
 * Requirement 12.2: Display recent activity including new restaurants, subscription changes, and user registrations
 */
const getRecentActivity = async (req, res) => {
  try {
      ...
      activities = activities.concat(
        users.map((u) => ({
          ...
          description: `New user "${u.name}" (${u.email}) registered${
            u.restaurant_names ? ` for ${u.restaurant_names}` : ""
          }`,
          user_id: u.id,
          user_name: u.name,
          user_email: u.email,
    ...
    return res.status(200).json(
      apiResponse.success(200, "Recent activity retrieved successfully", {
        data: activities,
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while retrieving recent activity")
    );
  }
};
 ...
 * Requirement 12.3: Provide charts showing growth trends over time
 * Requirement 12.6: Support date range selection
 */
const getAnalytics = async (req, res) => {
  try {
    ...
    if (!validIntervals.includes(interval)) {
      return res.status(400).json(
        apiResponse.error(400, "Invalid interval. Must be one of: day, week, month")
      );
    }
    ...
    return res.status(200).json(
      apiResponse.success(200, "Analytics data retrieved successfully", {
        data: {
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while retrieving analytics data")
    );
  }
};
 ...
 * Requirement 12.4: Display per-restaurant statistics including order volume and revenue
 */
const getRestaurantMetrics = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Restaurant metrics retrieved successfully", {
        data: {
          restaurant: {
            id: restaurant[0].id,
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while retrieving restaurant metrics")
    );
  }
};
