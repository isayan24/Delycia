...
const getAllRestaurants = async (req, res) => {
  try {
    ...
    const query = `
      ...
      FROM restaurants r
      LEFT JOIN subscription_assignments sa ON r.id = sa.restaurant_id AND sa.status = 'active'
      LEFT JOIN subscription_plans sp ON sa.subscription_plan_id = sp.id
      ${whereClause}
    ...
    return res.status(200).json(
      apiResponse.success(200, "Restaurants retrieved successfully", {
        data: restaurants,
        pagination: {
          page: parseInt(page),
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while retrieving restaurants")
    );
  }
};
...
const getRestaurant = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Restaurant details retrieved successfully", {
        data: result[0],
      })
    );
  } catch (error) {
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while retrieving restaurant details")
    );
  }
};
...
const createRestaurant = async (req, res) => {
  try {
    ...
    if (!name || !phone_number || !email || !address || !city || !state || !pincode) {
      return res.status(400).json(
        apiResponse.error(400, "Missing required fields: name, phone_number, email, address, city, state, pincode")
      );
    }
    ...
    if (existingName.length > 0) {
      return res.status(409).json(
        apiResponse.error(409, "Restaurant name already exists")
      );
    }
    ...
    return res.status(201).json(
      apiResponse.success(201, "Restaurant created successfully", {
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while creating restaurant")
    );
  }
};
...
const updateRestaurant = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Restaurant updated successfully", {
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while updating restaurant")
    );
  }
};
...
const deactivateRestaurant = async (req, res) => {
  try {
    ...
    if (existing[0].is_active === 0) {
      return res.status(400).json(
        apiResponse.error(400, "Restaurant is already inactive")
      );
    }
    ...
    return res.status(200).json(
      apiResponse.success(200, "Restaurant deactivated successfully", {
    ...
    return res.status(500).json(
      apiResponse.error(500, "An error occurred while deactivating restaurant")
    );
  }
};
