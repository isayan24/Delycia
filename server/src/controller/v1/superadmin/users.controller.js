...
const getAllUsers = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "Users retrieved successfully", {
        data: formattedUsers,
        pagination: {
          page: parseInt(page),
    ...
    return res
      .status(500)
      .json(
        apiResponse.error(500, "An error occurred while retrieving users")
      );
  }
};
...
const getUser = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "User details retrieved successfully", {
        data: {
          user,
          activity,
        },
  ...
  try {
    ...
    if (!name || !phone_number) {
      return res
        .status(400)
        .json(
          apiResponse.error(400, "Missing required fields: name, phone_number")
        );
    }
    ...
    // Check email uniqueness (platform-wide) - Requirement 4.8
    if (email) {
      ...
      if (existingEmail.length > 0) {
        return res
          ...
          .json(
            apiResponse.error(
              409,
              "Email address already exists in the platform"
            )
          );
      }
    }
    ...
    if (restaurant_ids && restaurant_ids.length > 0) {
      ...
      if (restaurants.length !== restaurant_ids.length) {
        return res
          .status(404)
          .json(
            apiResponse.error(404, "One or more restaurants not found")
          );
      }
      ...
      if (inactiveRestaurants.length > 0) {
        return res
          ...
          .json(
            apiResponse.error(
              400,
              "Cannot assign user to inactive restaurant(s)"
            )
          );
      }
    }
    ...
    return res.status(201).json(
      apiResponse.success(201, "User created successfully. Credentials have been sent to the user's email.", {
        data: userData,
      })
    );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(apiResponse.error(500, "An error occurred while creating user"));
  }
};
...
const updateUser = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "User updated successfully", {
        data: userData,
      })
    );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(apiResponse.error(500, "An error occurred while updating user"));
  }
};
...
const deactivateUser = async (req, res) => {
  try {
    ...
    if (existing[0].role === 0) {
      return res
        .status(400)
        .json(apiResponse.error(400, "User is already inactive"));
    }
    ...
    return res
      .status(500)
      .json(
        apiResponse.error(500, "An error occurred while deactivating user")
      );
  }
};
...
const resetPassword = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json(
        apiResponse.error(500, "An error occurred while resetting password")
      );
  }
};
...
const getUserActivity = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      apiResponse.success(200, "User activity retrieved successfully", {
        data: {
          user_id: parseInt(id),
