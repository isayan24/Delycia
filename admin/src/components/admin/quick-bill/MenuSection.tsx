...
export default function MenuSection({ addToCart, cart }: MenuSectionProps) {
  ...
  return (
    <div className="flex flex-col sidebar:h-full sidebar:overflow-hidden bg-background/50 sidebar:rounded-lg sidebar:border sidebar:border-slate-100 dark:sidebar:border-primary/5">
      {/* Search and Category Filter Header - FIXED via Flex & Full-Width on Mobile */}
      <div className="max-[900px]:sticky top-1 shrink-0 space-y-2 p-1 max-[900px]:-mx-5 max-[500px]:-mx-1 px-5 max-[500px]:px-1 bg-background/95 backdrop-blur-md pb-3 z-40 border-b border-slate-50 dark:border-primary/5 max-[900px]:py-2">
        {/* Search Bar - Modernized */}
        <div className="relative">
          <Search className="absolute left-8 max-[500px]:left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input
            placeholder="Search for items..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="pl-14 max-[500px]:pl-10 h-10 bg-white border shadow-sm focus-visible:ring-1 focus-visible:ring-primary/50"
          />
        </div>
        ...
        <Tabs
          ...
          className="w-full "
        >
          <div className="w-full overflow-x-auto pb-2 max-[900px]:pb-0 -mx-1 px-1 no-scrollbar scroll-smooth">
      ...
      {/* Item Grid - Stable Internal Scroll */}
            ...
            {filteredItems.map((item: Item) => {
              const quantity = (cart || [])
                .filter(
                  (c) =>
                    c.id === String(item.id) ||
                    c.id.startsWith(String(item.id) + '_'),
                )
