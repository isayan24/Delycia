...
export const DateFilterComponent: React.FC = () => {
  ...
  useEffect(() => {
    if (isCustomRangeOpen) {
      ...
      setTempStartDate(customStartDate ? customStartDate.toISOString().split('T')[0] : weekAgo.toISOString().split('T')[0]);
      setTempEndDate(customEndDate ? customEndDate.toISOString().split('T')[0] : today.toISOString().split('T')[0]);
    }
  ...
  const getCurrentOption = () => {
    return dateFilterOptions.find(option => option.value === selectedFilter) || dateFilterOptions[2];
  };
