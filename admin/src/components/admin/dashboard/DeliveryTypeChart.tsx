...
const getDeliveryColor = (type: string): string => {
  ...
  return (
    deliveryColors[normalizedType] ||
    deliveryColors[type.toLowerCase()] ||
    '#6b7280'
  )
}
...
const getDeliveryIcon = (type: string) => {
  ...
  if (
    normalizedType.includes('pickup') ||
    normalizedType.includes('takeaway')
  ) {
...
const CustomTooltip: React.FC<any> = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    const percentage = (
      (data.count / payload[0].payload.totalOrders) *
      100
    ).toFixed(1)
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3">
        ...
        <p className="text-gray-600 text-sm">
          Avg per Order:{' '}
          <span className="font-semibold">
            {formatCurrency(data.totalAmount / data.count)}
          </span>
        </p>
      </div>
    )
...
const DeliveryTypeLegend: React.FC<{ data: DeliveryTypeData[] }> = ({
  data,
}) => {
  ...
  return (
    <div className="mt-4 space-y-2">
      ...
      {data.map((delivery, index) => {
        const percentage =
          totalOrders > 0 ? (delivery.count / totalOrders) * 100 : 0
          ...
          >
            <div className="flex items-center space-x-2 flex-1 min-w-0">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span
                  className="text-gray-700 capitalize truncate"
                  title={delivery.type}
                >
                  {delivery.type}
                </span>
