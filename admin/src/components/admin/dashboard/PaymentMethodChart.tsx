...
import { CreditCard, AlertCircle, RefreshCw, Wallet } from 'lucide-react';
...
const getPaymentColor = (type: string): string => {
  const normalizedType = type.toLowerCase().replace(/[^a-z\s]/g, '');
  return paymentColors[normalizedType] || paymentColors[type.toLowerCase()] || '#6b7280';
};
...
const getPaymentIcon = (type: string) => {
  ...
  if (normalizedType.includes('wallet') || normalizedType.includes('pay')) {
  ...
  <div className="bg-white rounded-xl p-6 shadow-sm border border-red-200">
    ...
    <div className="h-[250px] flex items-center justify-center bg-red-50 rounded-lg">
      <div className="text-center">
        ...
        <p className="text-red-600 font-medium">Failed to load payment methods</p>
...
const CustomTooltip: React.FC<any> = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    const percentage = ((data.count / payload[0].payload.totalTransactions) * 100).toFixed(1);
    
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3">
        <div className="flex items-center space-x-2 mb-2">
          {getPaymentIcon(data.type)}
          <p className="text-gray-800 font-medium capitalize">{data.type}</p>
        </div>
        <p className="text-gray-600 text-sm">
          Transactions: <span className="font-semibold">{data.count.toLocaleString()}</span>
        </p>
        <p className="text-gray-600 text-sm">
          Amount: <span className="font-semibold">{formatCurrency(data.totalAmount)}</span>
        </p>
        <p className="text-gray-600 text-sm">
          Share: <span className="font-semibold">{percentage}%</span>
        </p>
        <p className="text-gray-600 text-sm">
          Avg per Transaction: <span className="font-semibold">{formatCurrency(data.totalAmount / data.count)}</span>
        </p>
      </div>
    );
  }
...
const PaymentMethodLegend: React.FC<{ data: PaymentMethodData[] }> = ({ data }) => {
  ...
  return (
    <div className="mt-4 space-y-2">
      ...
      {data.map((method, index) => {
        const percentage = totalTransactions > 0 ? (method.count / totalTransactions) * 100 : 0;
          ...
          <div key={index} className="flex items-center justify-between text-sm">
            <div className="flex items-center space-x-2 flex-1 min-w-0">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-gray-700 capitalize truncate" title={method.type}>
                  {method.type}
                </span>
              </div>
            ...
            <div className="flex space-x-6 text-right">
              <span className="font-medium w-16">{method.count.toLocaleString()}</span>
              <span className="font-medium w-20">{formatCurrency(method.totalAmount)}</span>
            </div>
          </div>
        );
      })}
      ...
      <div className="flex items-center justify-between text-sm font-semibold text-gray-800 border-t pt-2">
        <span>Total</span>
        <div className="flex space-x-6 text-right">
          <span className="w-16">{totalTransactions.toLocaleString()}</span>
...
export const PaymentMethodChart: React.FC<PaymentMethodChartProps> = ({
  ...
  const chartData = data.map(item => ({
    ...item,
    fill: getPaymentColor(item.type),
        ...
        <div>
          ...
          <div className="flex items-center space-x-4 mt-1">
            <span className="text-sm text-gray-600">
              Transactions: <span className="font-semibold text-blue-600">{totalTransactions.toLocaleString()}</span>
            </span>
            <span className="text-sm text-gray-600">
