...
export const useVariantManager = (
  ...
  useEffect(() => {
    if (
      ...
      JSON.stringify(initialVariants) !== JSON.stringify(variants)
    ) {
      setVariants(initialVariants);
      // Reset nextId based on existing variants
      setNextId(1);
    }
  }, [initialVariants]);
  ...
  const updateVariantName = (variantId: string | number, name: string): void => {
    setVariants((prev) =>
      prev.map((variant) =>
  ...
  const updateVariantPrice = (variantId: string | number, price: string): void => {
    setVariants((prev) =>
      prev.map((variant) =>
        variant.id.toString() === variantId.toString() 
          ? { ...variant, price } 
