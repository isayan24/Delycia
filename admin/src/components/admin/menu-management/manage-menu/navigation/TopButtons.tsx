...
interface TopButtonsProps {
  ...
  onNavigateToItem?: (itemId: string, type: "category" | "inventory") => void;
}

// Enhanced fuzzy search function for both categories and inventory items
const fuzzySearch = (query: string, categories: any[], inventoryItems: any[]): SearchResult[] => {
...
export const TopButtons = React.memo<TopButtonsProps>(
  ({ onSearch, onSubmit, onHighlightItem, onNavigateToItem }) => {
    ...
    useEffect(() => {
      ...
      setShowSuggestions(memoizedSearchResults.length > 0 && searchQuery.trim().length >= 2);
      setSelectedIndex(-1);
    ...
    const highlightMatch = (text: string, query: string) => {
      ...
      return parts.map((part, index) => 
        regex.test(part) ? (
          <span key={index} className="bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200 px-0.5 rounded font-medium">
            {part}
          </span>
        ) : part
      );
