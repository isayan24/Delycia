...
//todo check orders are recieving or not!!
...
export const useOrdersWebSocket = (
  ...
  const { user, isAuthenticated, accessToken, getValidAccessToken } = useAuth();
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const { token, rid, userId } = await getAuthData();
  ...
  useEffect(() => {
    if (isAuthenticated && socket?.connected && user) {
      ...
      getAuthData()
        .then(({ token, userId, rid }) => {
          socket.emit("auth_refresh", { token, userId, rid });
        })
        .catch((err) => {
          console.error("Failed to refresh auth:", err);
        });
    }
  }, [user, accessToken, isAuthenticated, socket, getAuthData]);
