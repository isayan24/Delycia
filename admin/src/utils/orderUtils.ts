...
import {
  ...
  convertToIST,
  getISTDateKey,
} from "@/components/admin/order-history/utils/historyDateUtils";

// mark Group orders by date, time, and customer - keeping each distinct order time separate
export const groupOrdersByDateAndCustomer = (
  orders: Order[]
): GroupedOrder[] => {
  ...
  const groupedArray: GroupedOrder[] = Array.from(orderGroups.entries()).map(
    ([key, groupOrders]) => {
