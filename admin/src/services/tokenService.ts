...
class TokenService {
  ...
  private async performTokenRefresh(): Promise<boolean> {
    try {
      ...
      // Based on your API structure, the refresh endpoint might be different
      ...
      if (response.data) {
        ...
        if (access_token && refresh_token) {
          ...
          sessionService.updateTokens(access_token, refresh_token);
          return true;
        } else {
          console.error("Invalid token refresh response structure");
          sessionService.clearSession();
          return false;
        }
      } else {
        console.error("Token refresh failed with status:", response.data);
        sessionService.clearSession();
        return false;
      }
    } catch (error: any) {
